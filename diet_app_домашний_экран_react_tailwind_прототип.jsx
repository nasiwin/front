...
// Inline data-URI background for the Diet card (temporary)
...
const exampleDiet = {
  ...
  name: "Кето",
  kcalTarget: 1800,
  macros: { p: 130, f: 120, c: 30 }, // g/day
  rules: {
    maxSugar: 10,
    maxNetCarbs: 30,
    avoid: ["рафинированный сахар", "сладкие напитки", "белый хлеб"],
  },
};

const exampleDayStats = {
  kcal: 920,
  macros: { p: 68, f: 72, c: 14 },
};

const exampleTrips = [
  {
    id: "trip-1",
    title: "Покупка — Магнит (09:20)",
    items: [
...
function suggestAlternatives(product: any, diet: any) {
  ...
  // Very naive suggestions — replace carb-heavy sweets with nuts/dark chocolate
...
function compensationsForBasket(trips: any[], diet: any) {
  // Simple imbalance check: if carbs overshoot relative to target share, suggest protein/fat sources
  ...
  if (totals.c > diet.macros.c * 0.6) {
    out.push({ name: "Тунец в собственном соку 1 банка", reason: "Компенсирует избыток углей белком" });
  }
  if (totals.p < diet.macros.p * 0.6) {
    out.push({ name: "Греческий йогурт 200г", reason: "Добавит белка" });
  }
  if (totals.f < diet.macros.f * 0.6) {
    out.push({ name: "Авокадо 1 шт.", reason: "Полезные жиры" });
  }
  return out;
}

function getDietCover(diet?: { id?: string }) {
  const covers: Record<string, string> = {
    keto: "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?auto=format&fit=crop&w=1200&q=60",
    paleo: "https://images.unsplash.com/photo-1512621776951-4b22e7f0b0c3?auto=format&fit=crop&w=1200&q=60",
    vegan: "https://images.unsplash.com/photo-1506806732259-39c2d0268443?auto=format&fit=crop&w=1200&q=60",
  };
  return covers[diet?.id ?? ""] || "https://images.unsplash.com/photo-1543352634-8732c0f3cf52?auto=format&fit=crop&w=1200&q=60";
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 pb-28">
      ...
      <main className="px-4 pt-4 space-y-4 max-w-md mx-auto">
        <DietCard diet={diet} onOpenSettings={() => { /* navigate("/diet/settings") */ }} />

        <div className="grid grid-cols-2 gap-4">
          <StatsCard stats={stats} diet={diet} />
          <BasketCard trips={trips} onOpenBasket={() => { /* navigate("/basket/today") */ }} />
        </div>
      </main>
      ...
      <ScannerDialog
        ...
        onManualSubmit={(code: string) => {
          const mock = mockByBarcode(code);
          setScanResult(mock);
        }}
      >
        ...
        <Sheet>
          ...
          <SheetContent side="right" className="w-[90vw] max-w-sm">
            ...
            <div className="mt-4 text-sm text-slate-600 space-y-3">
              {/* Дополнительные элементы настроек будут здесь */}
            </div>
          </SheetContent>
        </Sheet>
      </div>
...
function DietCard({ diet, onOpenSettings }: { diet: typeof exampleDiet; onOpenSettings: () => void }) {
  return (
    <Card className="rounded-2xl overflow-hidden relative shadow-sm cursor-pointer min-h-20" onClick={onOpenSettings}>
      {/* Place the file at /public/keto-bg.png */}
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      try {
        ...
        setActive(true);
        // Simple polling detector (placeholder). Production: use requestAnimationFrame & draw to canvas for performance
        // @ts-ignore
        ...
        const tick = async () => {
          ...
          try {
            ...
            if (barcodes && barcodes[0]) {
              onManualSubmit(barcodes[0].rawValue);
            }
          } catch {}
        ...
        <div className="space-y-3">
          {supported ? (
            ...
            <div className="p-4 border rounded-xl text-sm text-slate-600">
              Ваш браузер не поддерживает BarcodeDetector. Введите код вручную ниже.
            </div>
          )}

          <div className="flex items-center gap-2">
            <Input placeholder="Введите штрихкод" onKeyDown={(e) => { if (e.key === "Enter") onManualSubmit((e.currentTarget as HTMLInputElement).value); }} />
            <Button onClick={() => {
              const el = document.querySelector("input[placeholder='Введите штрихкод']") as HTMLInputElement | null;
              if (el) onManualSubmit(el.value);
            }}>
...
function ScanResultView({ result, diet, onAdd }: { result: any; diet: typeof exampleDiet; onAdd: (p: any) => void }) {
  if (!result) {
    return <div className="text-sm text-slate-500">Наведите камеру на штрихкод или введите код.</div>;
  }
    ...
    <div className="space-y-1">
      ...
      {result.fit === "warn" && (
        <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2">
          Причина: {result.reason || "может нарушить правила диеты"}
        </div>
      )}
      <div className="flex items-center gap-2">
...
function mockByBarcode(code: string) {
  if (!code) return null;
  if (/^460/.test(code)) {
    return { id: code, name: "Шоколад молочный 90г", qty: 1, kcal: 480, p: 6, f: 28, c: 52, fit: "warn", reason: "Сахар/углеводы" };
  }
...
// -----------------------------
// TEST HOOK (exported helpers) & SAMPLE TEST CASES
// -----------------------------
export const __testables = { clamp, pct, suggestAlternatives, compensationsForBasket, getDietCover, mockByBarcode };

/*
Example test cases (Vitest / Jest):

import { describe, it, expect } from 'vitest';
import HomeScreen, { __testables } from './index';

const { clamp, pct, suggestAlternatives, compensationsForBasket, getDietCover, mockByBarcode } = __testables;
...
describe('utils', () => {
  ...
  it('pct computes percentage safely', () => {
    expect(pct(50, 200)).toBe(25);
    expect(pct(0, 0)).toBe(0);
  });
  it('suggestAlternatives returns empty for fit ok', () => {
    expect(suggestAlternatives({ name: 'Творог', fit: 'ok' }, {})).toEqual([]);
  });
  it('suggestAlternatives returns options for chocolate', () => {
    const res = suggestAlternatives({ name: 'Шоколад молочный', fit: 'warn' }, {});
    expect(res.length).toBeGreaterThan(0);
  });
  it('compensationsForBasket suggests when carbs high', () => {
    const trips = [{ items: [{ kcal: 200, p: 2, f: 2, c: 60, qty: 1 }] }];
    const diet = { macros: { p: 130, f: 120, c: 30 } } as any;
    const tips = compensationsForBasket(trips as any, diet);
    expect(tips.some(t => t.name.includes('Тунец'))).toBe(true);
  });
  it('getDietCover returns default for unknown', () => {
    expect(getDietCover({ id: 'unknown' })).toContain('images.unsplash');
  });
  it('mockByBarcode returns warn for 460*', () => {
    expect((mockByBarcode('460123') as any).fit).toBe('warn');
  });
  it('mockByBarcode returns ok for non-460', () => {
    expect((mockByBarcode('123456') as any).fit).toBe('ok');
  });
});
*/
